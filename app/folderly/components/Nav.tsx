...
export function TopNav({ left, right, className }: { left?: React.ReactNode; right?: React.ReactNode; className?: string }) {
  return (
    <header className={cn('h-16 top-0 z-10 flex items-center justify-between px-4 backdrop-blur border-b border-[var(--color-border)] bg-[color-mix(in_srgb, var(--color-surface)_90%, transparent)]', className)}>
